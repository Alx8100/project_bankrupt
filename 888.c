...
int getInsuranceAdvice(double trueCount) {
    return trueCount >= 3; // Typically, insurance is advised if the true count is 3 or higher
}
...
void initializeGame(int *balance, int *baseBet, double *cardCount) {
    int decks = 8; // Fixed number of decks
    printf("Number of decks: %d (4 decks in play)\n", decks);
...
double calculateTrueCount(double cardCount, char *seenCards) {
    int remainingDecks = 4 - (int)strlen(seenCards) / 52; // Only 4 decks are actively in play
...
void handleSplitHands(char *playerCards, int dealerCardValue, double trueCount, int *currentBet, int *playerScore, double *cardCount, int *isSoft, int *numAces) {
    ...
    *numAces = (firstHandCard1 == 'A') ? 1 : 0;
    ...
    while (1) {
        ...
        updatePlayerScore(playerScore, nextCardValue, isSoft, numAces);
        *cardCount += wong_halves(nextCard);
        Action action = getAction(*playerScore, dealerCardValue, *isSoft, 0, firstHandCard1, nextCard, trueCount, hasHit);
        if (action == STAND || *playerScore > 21) {
            break;
        }
        if (action == DOUBLE && !hasHit) {
            printf("Player should DOUBLE.\n");
            *currentBet *= 2;
            printf("Enter the next card for the first hand: ");
            scanf(" %c", &nextCard);
    ...
    *numAces = (secondHandCard1 == 'A') ? 1 : 0;
    ...
    while (1) {
        ...
        updatePlayerScore(playerScore, nextCardValue, isSoft, numAces);
        *cardCount += wong_halves(nextCard);
        Action action = getAction(*playerScore, dealerCardValue, *isSoft, 0, secondHandCard1, nextCard, trueCount, hasHit);
        if (action == STAND || *playerScore > 21) {
            break;
        }
        if (action == DOUBLE && !hasHit) {
...
int main() {
    char playerCards[5], dealerCard, seenCards[208] = {0}; // 208 cards in 4 decks
    double cardCount = 0;
    int baseBet;
    int balance;

    initializeGame(&balance, &baseBet, &cardCount);
    ...
    while (balance > 0) {
        ...
        double trueCount = calculateTrueCount(cardCount, seenCards); // Only 4 decks are actively in play
        ...
        if (dealerCard == 'A') {
            int insuranceAdvice = getInsuranceAdvice(trueCount);
