...
int main() {
    char playerCards[5], dealerCard, seenCards[208] = {0}; // 416 cards in 8 decks
    int cardCount = 0;
    int baseBet;
    int balance;
    int decks = 4; // Number of decks in the shoe
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (action == HIT) {
                ...
                while (1) {
                    ...
                    updatePlayerScore(&playerScore, nextCardValue, &isSoft, &numAces);
                    cardCount += hi_lo(nextCard);
                    action = getAction(playerScore, dealerCardValue, isSoft, 0, firstHandCard1, nextCard, trueCount);
                    if (action == STAND || playerScore > 21) {
                        break;
                    }
                    if (action == DOUBLE) {
                ...
                while (1) {
                    ...
                    updatePlayerScore(&playerScore, nextCardValue, &isSoft, &numAces);
                    cardCount += hi_lo(nextCard);
                    action = getAction(playerScore, dealerCardValue, isSoft, 0, secondHandCard1, nextCard, trueCount);
                    if (action == STAND || playerScore > 21) {
                        break;
                    }
                    if (action == DOUBLE) {
        ...
        if (playerScore > 21) {
            ...
            printf("Playeer stands with score %d.\n", playerScore);
        }
